...
module float_complex_div_divopt(
    input clk,                //����ʱ���ź�        
    input rst_n,              //���븴λ�ź�
    input start,              //���뿪ʼ�ź�
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            div_start <= start && (state == 3'd3) && !div_busy1 && !div_busy2;
        end
    end
    always @(posedge clk or negedge rst_n) begin
        if(!rst_n) begin
    ...
    asic_signed_fractional_divider_radix4_aligned u1_div (
        .clk(clk),
        .rst_n(rst_n),
        .start(div_start),
        .dividend($signed(result8)),
        .divisor($signed(result9)),
        .busy(div_busy1),
        .done(),
        .valid(div_rdy1),
        .quotient(out10),
        .divide_by_zero(div_zero1)
    );//72Q32



    asic_signed_fractional_divider_radix4_aligned u2_div (
        .clk(clk),
        .rst_n(rst_n),
        .start(div_start),
        .dividend($signed(result7)),
        .divisor($signed(result9)),
        .busy(div_busy2),
        .done(),
        .valid(div_rdy2),
        .quotient(out11),
        .divide_by_zero(div_zero2)
    );//72Q32
